...
export function HyperText({
  ...
  return (
    <motion.div  // ✅ FIX: Used motion.div instead of motion.create(Component)
      ref={elementRef}
      className={cn("overflow-hidden py-2 text-4xl font-bold", className)}
      style={{ fontSize: "85px", cursor: "pointer" }}  // ✅ FIX: Added cursor pointer
      onMouseEnter={handleAnimationTrigger}
      onClick={(e) => {
        e.stopPropagation(); // ✅ Prevent event bubbling issues
        if (props.onClick) {
          props.onClick(); // ✅ Ensure the onClick function is correctly executed
        }
      }}
      {...props}
    >
